...
DesktopManager::DesktopManager(char *loc, char *imgloc, int nLEDs)
{
	...
	image_width = DESKTOP_IMAGE.width();
	image_height = DESKTOP_IMAGE.height();
	
	if((arduino_fd = open(loc, O_RDWR | O_NOCTTY)) < 0) {
		perror(loc);
		exit(EXIT_FAILURE);
	}
	printf("Opening communications with Arduino on %s... ", loc);
	...
	tcsetattr(arduino_fd, TCSANOW, &new_io);
...
bool DesktopManager::mainLoop(void)
{
	while(true);
